...
case ${CS_MAIL_BILLING_INSTANT_ACTIVATION:-false} in true|false) ;; *) fail "CS_MAIL_BILLING_INSTANT_ACTIVATION must be true or false" ;; esac
[[ ${CS_MAIL_ENVIRONMENT:-production} == production ]] || fail "CS_MAIL_ENVIRONMENT must be production when set"
if [[ ${CS_MAIL_BILLING_INSTANT_ACTIVATION:-false} == true ]]; then
  echo "WARNING: CS_MAIL_BILLING_INSTANT_ACTIVATION=true — acceptance-test mode is active; unpaid test orders can activate immediately. Public launch-freeze will remain blocked until this is false." >&2
fi
