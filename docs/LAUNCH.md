...
- [ ] Customer self-delete flow live (the other half of LAUNCH:191): backend route `/api/account/delete` + password-reconfirm handler exist and are **real** (`account.rs:delete_self`, argon verified, audit `account.erase`), but **NO frontend control POSTs it** (`(C)=0` files) and **no blocker leg drives the customer round trip** (`(B)=0` legs) — needs a Security/Settings page delete-my-account button wired to the real POST + a blocking `api_flows` self-delete→401 leg.
