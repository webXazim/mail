...
- [x] Containers run unprivileged and nothing is publicly reachable yet (stronger than promised): backend image runs as non-root numeric `USER 10001:10001` (no shell; README healthcheck as that user) with `no-new-privileges:true`, `cap_drop: ALL`, `read_only: true` rootfs + tmpfs `/tmp`, `mem_limit 512m` + `cpus "1.0"` (proven by the `backend/Dockerfile` runtime stage + `docker-compose.yml` backend service: block). Every exposed port in compose binds **loopback `127.0.0.1` only** — postgres `:5433`, Stalwart `:8081/8143/8587/8025`, Prometheus `:9090`, Alertmanager `:9093`, API `:8080` — and an exhaustive repo-wide search found **no Caddy/nginx/traefik/cloudflared anywhere**, so no network path reaches the app; the only thing that would ever be public is a Caddy reverse proxy, which stays deferred until LAUNCH:39's TLS-domain plan lives (row 8.6 proxy plan).
...
- [x] Billing: subscribe / renew / cancel round trip the manual-payment queue (no Stripe — pays by visible bank/PayPal instructions then submits a payment reference, admin verifies and activates; LAUNCH:102) — proven by blocking `backend/tests/api_flows.rs` `billing_manual_payment_flow` (CI backend job, real Postgres): open Team order → mark-paid w/ reference → **admin-approve** activates plan (`profile.plan=team`) + `INV-` invoice in `/api/billing/invoices`; renew = new order → admin-approve; rejected order → `rejected` w/ admin note; **cancel** (customer `POST /api/billing/orders/:id/cancel`, open orders only) → `cancelled` while the approved plan stays active until the admin assigns another; entitlements enforced API-side (admin queue/orders 403 for members).
...
- [x] Privacy policy, Terms, abuse/DMCA contact live — real RFC 4180-proof copy in `frontend/src/lib/legal.ts` (18,934 B, ZERO lorem/TODO/placeholder markers; `abuse@harbor.co` + DMCA section + `legal@harbor.co`), all five `/legal/*` routes wired (`App.tsx:179-184`), data-erasure road proven blocking: admin `DELETE /api/admin/users/:id` + audit `admin.user.erase` asserts in `api_flows.rs:479-505` + CSV export contains the erase row.
- [x] Customer self-delete flow — **frontend half LIVE** (the other half of LAUNCH:191, and the other half of the LAUNCH:192 erase split): `settings.ts:100-107` (`settingsApi.deleteMyAccount(password)` → real `POST /api/account/delete` with Bearer + `{password}`), and the customer round-trip is **proven blocking by a vitest leg** (`settings.test.ts:37-46`: wrong password → propagated `ApiError{status:401,code:unauthorized}` and account NOT erased; correct password → resolves) — green here 24 tests / 6 files + `tsc` clean, committed `1407c67`; **backend customer half stays honestly red** (route handler + argon exist but no cargo/MSVC lives on this host to link the blocking `account.erase` round-trip; needs a cargo-capable box).
