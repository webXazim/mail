...
import {
  AtSign, Check, Copy, FileWarning, Globe, History as HistoryIcon, KeyRound, LayoutDashboard, Mailbox, RefreshCw, Search, ShieldAlert, ShieldCheck, Trash2,
} from 'lucide-react'
import { adminApi, dnsRecords, type Alias, type AuditEntry, type MailboxAccount, type QuarantinedMail, type SecuritySettings } from '../services/admin'
import { identitiesApi } from '../services/identities'
import { useMail } from '../state/mail/MailContext'

type AdminTab = 'overview' | 'mailboxes' | 'aliases' | 'forwarders' | 'domain' | 'security' | 'quarantine' | 'audit'
...
const timeFmt = (iso: string) => new Date(iso).toLocaleString([], { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })
...
export function AdminPage() {
  ...
  const [mailboxStatusFilter, setMailboxStatusFilter] = useState<'all' | MailboxAccount['status']>('all')
  ...
  const visibleMailboxes = mailboxes.filter(mailbox => {
    ...
    const matchesQuery = !query || `${mailbox.email} ${mailbox.displayName}`.toLowerCase().includes(query)
  ...
  const addMailbox = (event: FormEvent) => {
    ...
    const current = adminApi.addMailbox({ email: raw, displayName: mailboxForm.displayName, quotaGB: mailboxForm.quotaGB })
  ...
  const updateSecurityAndLog = (patch: Partial<SecuritySettings>, action: string, detail: string) => {
  ...
  const addBlockedSender = (event: FormEvent) => {
    ...
    if (next === security) { showNotice('That sender is already blocked'); return }
    setSecurity(next)
    setAudit(adminApi.listAudit())
    setBlockedForm('')
  ...
  const recordValue = (record: { id: string; value: string }) => (record.id === 'dmarc' ? `_dmarc.${domain.domain}. TXT "v=DMARC1; p=${security.dmarcPolicy}; rua=mailto:dmarc@${domain.domain}"` : record.value)
  const dmarcLabel = { none: 'No action', quarantine: 'Quarantine', reject: 'Reject' }[security.dmarcPolicy]
  ...
  return (
    <div className="admin-page" role="region" aria-label="Admin center">
      ...
      {tab === 'overview' && (
        <>
          <div className="admin-stats">
            <div className="admin-stat"><strong>{activeCount}</strong><small>Active mailboxes</small></div>
            <div className="admin-stat"><strong>{quarantine.length}</strong><small>Quarantined messages</small></div>
            <div className="admin-stat"><strong>{aliases.length}</strong><small>Aliases</small></div>
            <div className="admin-stat"><strong>{forwarders.length}</strong><small>Forwarders</small></div>
            <div className="admin-stat"><strong>{mailboxes.filter(mailbox => mailbox.status === 'quarantine').length}</strong><small>Quarantined mailboxes</small></div>
            <div className="admin-stat"><strong>{totalUsed.toFixed(1)} / {totalQuota} GB</strong><small>Storage used</small></div>
          </div>
          ...
          <section className="settings-section">
            ...
            <button type="button" className="secondary-button" onClick={() => setTab('audit')}><HistoryIcon size={14} />View full audit log</button>
          </section>
        </>
      )}
        ...
        <>
          <section className="settings-section">
            <div className="admin-section-head">
              <h3>Mailboxes</h3>
              <span className="admin-section-count">{visibleMailboxes.length} of {mailboxes.length}</span>
            </div>
            <div className="admin-list-tools">
              <label className="admin-search">
                <Search size={14} />
                <input value={mailboxQuery} onChange={event => setMailboxQuery(event.target.value)} placeholder="Search mailboxes" aria-label="Search mailboxes" />
              </label>
              <select value={mailboxStatusFilter} onChange={event => setMailboxStatusFilter(event.target.value as typeof mailboxStatusFilter)} aria-label="Filter mailboxes by status">
            ...
            {visibleMailboxes.map(mailbox => (
              <div className="billing-row" key={mailbox.id}>
                <div><strong>{mailbox.email}</strong><small>{mailbox.displayName} · {mailbox.storageUsedGB.toFixed(1)} GB of {mailbox.quotaGB} GB used</small></div>
                <div className="admin-actions">
                  <select className="admin-status-select" value={mailbox.status} aria-label={`Status for ${mailbox.email}`} onChange={event => changeStatus(mailbox.id, event.target.value)}>
                    <option value="active">Active</option>
                    <option value="quarantine">Quarantine</option>
                    <option value="disabled">Disabled</option>
                  </select>
                  <input className="admin-quota-input" type="number" min={1} max={100} defaultValue={mailbox.quotaGB} aria-label={`Quota for ${mailbox.email}`} onBlur={event => changeQuota(mailbox.id, event.target.value)} title="Storage quota (GB)" />
                  <button type="button" className="secondary-button" onClick={() => resetPassword(mailbox.id)}><KeyRound size={13} />Reset</button>
            ...
            {visibleMailboxes.length === 0 && <p className="settings-hint">No mailboxes match this search.</p>}
          </section>
          <form className="settings-section" onSubmit={addMailbox}>
            ...
            <label>Storage quota (GB)<input type="number" min={1} max={100} value={mailboxForm.quotaGB} onChange={event => setMailboxForm(current => ({ ...current, quotaGB: Number(event.target.value) || 10 }))} aria-label="Mailbox quota (GB)" /></label>
          ...
          <section className="settings-section">
            ...
            {dnsRecords.map(record => (
              <div className="billing-row" key={record.id}>
                <div><strong>{record.name}</strong><small className="admin-record">{recordValue(record)}</small></div>
                ...
                <button type="button" className="secondary-button" onClick={() => void copyRecord({ id: record.id, value: recordValue(record) })}>{copied === record.id ? <Check size={14} /> : <Copy size={14} />}{copied === record.id ? 'Copied' : 'Copy'}</button>
              </div>
            ))}
          </section>
          <section className="settings-section">
      ...
      {tab === 'security' && (
        <>
          <section className="settings-section">
            <h3>Spam and deliverability</h3>
            <label className="admin-range">
              <span>Spam sensitivity — {security.spamThreshold} / 10</span>
              <input type="range" min={1} max={10} step={1} value={security.spamThreshold} aria-label="Spam threshold" onChange={event => updateSecurityAndLog({ spamThreshold: Number(event.target.value) }, 'Spam threshold changed', `${event.target.value} / 10`)} />
            </label>
            <div className="settings-options">
              <label><input type="checkbox" checked={security.requireTls} onChange={event => updateSecurityAndLog({ requireTls: event.target.checked }, event.target.checked ? 'Inbound TLS enforced' : 'Inbound TLS allowed', domain.domain)} /> Require TLS for inbound mail</label>
              <label><input type="checkbox" checked={security.scanAttachments} onChange={event => updateSecurityAndLog({ scanAttachments: event.target.checked }, event.target.checked ? 'Attachment scanning enabled' : 'Attachment scanning disabled', '')} /> Scan attachments for malware</label>
            </div>
            <label>Retention policy
              <select value={security.retentionDays} aria-label="Retention policy" onChange={event => updateSecurityAndLog({ retentionDays: Number(event.target.value) }, 'Retention policy changed', retentionLabel(Number(event.target.value)))}>
            ...
            <label>DMARC policy
              <select value={security.dmarcPolicy} aria-label="DMARC policy" onChange={event => updateSecurityAndLog({ dmarcPolicy: event.target.value as SecuritySettings['dmarcPolicy'] }, 'DMARC policy changed', event.target.value)}>
                <option value="none">No action</option>
                <option value="quarantine">Quarantine</option>
                <option value="reject">Reject</option>
              </select>
            </label>
            <p className="settings-hint">DMARC policy is <strong>{dmarcLabel}</strong> — the DNS record above reflects this choice.</p>
          </section>
          <section className="settings-section">
            <h3>Blocked senders</h3>
            {security.blockedSenders.map(sender => (
              <div className="billing-row" key={sender}>
                <div><strong>{sender}</strong><small>Blocked from delivering to any mailbox</small></div>
                <button type="button" className="icon-button" aria-label={`Remove blocked sender ${sender}`} onClick={() => removeBlockedSender(sender)}><Trash2 size={15} /></button>
              </div>
            ))}
            {security.blockedSenders.length === 0 && <p className="settings-hint">No senders blocked — add one below.</p>}
            <form className="admin-inline-form" onSubmit={addBlockedSender}>
              <input value={blockedForm} onChange={event => setBlockedForm(event.target.value)} placeholder="sender@example.com" aria-label="Blocked sender" />
              <button type="submit" className="primary-button"><ShieldAlert size={15} />Block sender</button>
            </form>
          </section>
        </>
      )}
      ...
      {tab === 'quarantine' && (
        <section className="settings-section">
          <h3>Quarantined messages</h3>
          {quarantine.map(item => (
            <div className="billing-row" key={item.id}>
              <div><strong>{item.subject}</strong><small>{item.from} → {item.to} · {item.reason} · {item.sizeKB} KB</small></div>
              <span className="admin-audit-time">{timeFmt(item.date)}</span>
              <div className="admin-actions">
                <button type="button" className="secondary-button" onClick={() => releaseQuarantine(item.id)}><ShieldCheck size={13} />Release</button>
                <button type="button" className="icon-button" aria-label={`Delete quarantined message ${item.subject}`} onClick={() => removeQuarantine(item.id)}><Trash2 size={15} /></button>
              </div>
            </div>
          ))}
          {quarantine.length === 0 && <p className="settings-hint">Nothing quarantined right now.</p>}
        </section>
      )}
