...
export function MailListPage() {
  ...
  const scheduledById = useMemo(
    () => new Map(scheduleApi.list().map((entry) => [entry.id, entry])),
    [],
  )
  ...
  const filtered = useMemo(() => {
    if (folder === 'Scheduled')
      return scheduleApi.list().slice(0, scheduledCount).map(buildScheduledMail)
        ...
        {loading ? (
          ...
          visible.map((mail, index) => (
            <MailRow
              ...
              onQuickCancel={
                readOnly && folder === 'Scheduled' ? (mail) => removeScheduled(mail.id) : undefined
              }
              onLongPress={
                readOnly
                  ? undefined
