...
export function CalendarPage() {
  ...
  const save = (draft: Omit<CalendarEvent, 'id'>, existing: CalendarEvent | null) => {
    const normalized = { ...draft, title: draft.title.trim() || 'Untitled event' }
    setEvents(existing ? calendarApi.update(existing.id, normalized) : calendarApi.add(normalized))
    setEditor(null)
    showNotice(existing ? 'Event updated' : 'Event created')
  }

  const remove = (event: CalendarEvent) => {
    setEvents(calendarApi.remove(event.id))
    setEditor(null)
