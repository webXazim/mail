...
const parsedSentDate = (mail: Mail): Date | null => {
  ...
  if (md && monthStart[md[1]] !== undefined)
    return new Date(now.getFullYear(), monthStart[md[1]], Number(md[2]))
  return null
}
...
const dateComparison = (
  mail: Mail,
  condition: Extract<RuleCondition, { field: 'date' }>,
): boolean => {
...
const conditionPasses = (mail: Mail, condition: RuleCondition): boolean => {
  switch (condition.field) {
    ...
    case 'to':
      return (
        mail.email.toLowerCase().includes(condition.value.toLowerCase()) ||
        (mail.to ?? []).some((address) =>
          address.toLowerCase().includes(condition.value.toLowerCase()),
        )
      )
    case 'subject':
    ...
    case 'size':
      return condition.op === 'larger'
        ? estimatedSizeKb(mail) > condition.size
        : estimatedSizeKb(mail) < condition.size
    case 'date':
...
export function applyIncomingFilters(mailbox: Mail[]): { mailbox: Mail[]; report: PipelineReport } {
  ...
  let next = mailbox.map((mail) => {
    ...
    if (allowed.has(sender) && (mail.folder || 'Inbox') === 'Spam')
      return { ...mail, folder: 'Inbox' }
    return mail
  })
