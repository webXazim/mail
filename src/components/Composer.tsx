...
const parseAddresses = (value: string) => value.split(',').map(part => part.trim()).filter(Boolean)
...
export function Composer({ close, onSent, initialDraft }: ComposerProps) {
  ...
  const [fromIdentity, setFromIdentity] = useState(identities[0].email)
