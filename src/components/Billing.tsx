...
const plans = [
  { id: 'solo', name: 'Harbor Solo', price: '$0', seats: 1, detail: 'For personal use' },
  { id: 'team', name: 'Harbor Team', price: '$8', seats: 5, detail: 'Per seat / month' },
  { id: 'business', name: 'Harbor Business', price: '$16', seats: 25, detail: 'Per seat / month' },
]
...
export function Billing({ close }: { close: () => void }) {
  ...
  return (
    <div className="settings-layer" role="presentation">
      <section ref={panelRef} className="settings-panel" role="dialog" aria-modal="true" aria-labelledby="billing-title">
        <header>
          <div><p className="eyebrow">Harbor Mail</p><h2 id="billing-title">Billing</h2></div>
          <button type="button" className="icon-button" aria-label="Close billing" onClick={close}><X size={17} /></button>
        </header>
        <nav className="settings-nav" aria-label="Billing sections">
          <button type="button" className={tab === 'plan' ? 'settings-nav--active' : ''} aria-current={tab === 'plan' ? 'page' : undefined} onClick={() => setTab('plan')}><CreditCard size={15} />Plan</button>
          <button type="button" className={tab === 'invoices' ? 'settings-nav--active' : ''} aria-current={tab === 'invoices' ? 'page' : undefined} onClick={() => setTab('invoices')}><FileText size={15} />Invoices</button>
        </nav>
            ...
            <>
              <div className="settings-section">
                <div className="billing-plan">
                  <div><h3>Current plan</h3><strong>{plans.find(item => item.id === plan)?.name}</strong><small><span>${plans.find(item => item.id === plan)?.price} per seat / month</span> · Renews October 1, 2026</small></div>
                ...
                <div className="storage"><span>Seats used</span><strong>1 <small>/ {plans.find(item => item.id === plan)?.seats}</small></strong><div className="storage-bar"><span style={{ width: `${(1 / (plans.find(item => item.id === plan)?.seats ?? 5)) * 100}%` }} /></div></div>
              </div>
              {changing && (
                <div className="settings-section">
                  <h3>Choose a plan</h3>
