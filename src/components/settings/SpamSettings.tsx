...
export function SpamSettings() {
  ...
  const block = () => {
    if (!matchingEmail(blockedInput)) {
      setNotice('Enter a valid email address to block.')
      return
    }
  ...
  const allow = () => {
    if (!matchingEmail(allowedInput)) {
      setNotice('Enter a valid email address to allow.')
      return
    }
    ...
    <div>
      <div className="settings-section">
        ...
        <label>
          Protection level
          <select
            value={settings.spamLevel}
            aria-label="Junk filter level"
            onChange={(event) =>
              setSettings(spamApi.save({ ...settings, spamLevel: event.target.value as SpamLevel }))
            }
          >
      ...
      <div className="settings-section">
        ...
        <div className="spam-add">
          ...
          />
          <button type="button" className="primary-button" onClick={block}>
            <ShieldOff size={15} />
            Block
          </button>
        </div>
        {settings.blocked.map((address) => (
          <div className="billing-row" key={address}>
            <div>
              ...
              <small>Blocked sender</small>
            </div>
            <button
              type="button"
              className="icon-button"
              aria-label={`Unblock ${address}`}
              onClick={() => setSettings(spamApi.removeBlocked(address))}
            >
              <Trash2 size={15} />
            </button>
          </div>
      ...
      <div className="settings-section">
        ...
        <div className="spam-add">
          ...
          />
          <button type="button" className="primary-button" onClick={allow}>
            <Plus size={15} />
            Allow
          </button>
        </div>
        {settings.allowed.map((address) => (
          <div className="billing-row" key={address}>
            ...
            <button
              type="button"
              className="icon-button"
              aria-label={`Remove ${address} from allow list`}
              onClick={() => setSettings(spamApi.removeAllowed(address))}
            >
              <Trash2 size={15} />
            </button>
          </div>
