...
export const identitiesApi = {
  ...
  add(identity: { email: string; displayName: string }) {
    ...
    if (!email.includes('@') || this.list().some(item => item.email.toLowerCase() === email)) return this.list()
    const next = [...this.list(), { id: `identity-${Date.now()}`, email, displayName: identity.displayName.trim() || identity.email.split('@')[0], primary: false }]
    this.save(next)
    return next
  },
  rename(id: string, displayName: string) {
    const next = this.list().map(identity => (identity.id === id ? { ...identity, displayName } : identity))
    this.save(next)
    return next
  },
