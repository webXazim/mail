...
const mailboxKeyFor = (accountId: string) => (accountId === primaryAccountId ? mailboxKey : `harbor-mail:mailbox:${accountId}`)
const delay = (value: Mail[] | null) => new Promise<Mail[]>(resolve => window.setTimeout(() => resolve(value || messages), 120))
const delayList = (value: Mail[] | null) => new Promise<Mail[]>(resolve => window.setTimeout(() => resolve(value ?? []), 120))
...
export const mailboxApi = {
  ...
  async replaceFor(accountId: string, next: Mail[]) { localStorage.setItem(mailboxKeyFor(accountId), JSON.stringify(next)); return next },
