...
describe('LoginPage', () => {
  ...
  it('signs in, records the audit event and navigates to the mailbox', async () => {
    ...
    vi.mocked(authApi.login).mockResolvedValue({
      access: 'tok',
      refresh: 'ref',
      user: { id: 'u1', email: 'alex@harbor.test', display_name: 'Alex', role: 'admin' },
    })
    mount()

    await user.type(screen.getByLabelText('Email address'), 'alex@harbor.test')
    await user.type(screen.getByLabelText('Password'), 'Strong-Pass!1')
    await user.click(screen.getByRole('button', { name: /sign in/i }))

    expect(authApi.login).toHaveBeenCalledWith('alex@harbor.test', 'Strong-Pass!1')
