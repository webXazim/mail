...
pub async fn mailbox_action(State(state):State<AppState>,admin:AdminUser,Path(mailbox_id):Path<Uuid>,Json(body):Json<MailboxAction>)->Result<Json<Value>,ApiError>{
    ...
    match body.action.as_str(){
      ...
      "set_quota"=>{
        ...
        let mut tx=state.db.begin().await.map_err(|e|ApiError::internal(e.to_string()))?;
        let pool:i64=sqlx::query_scalar("SELECT COALESCE(s.storage_pool_override_bytes,COALESCE(pv.storage_pool_bytes,p.storage_pool_bytes)::bigint+COALESCE(pv.mailbox_bytes,p.mailbox_bytes)::bigint*GREATEST(s.purchased_mailbox_count-COALESCE(pv.mailbox_limit,p.mailbox_limit),0)::bigint)::bigint FROM organization_subscriptions s JOIN plans p ON p.code=s.plan_code LEFT JOIN plan_versions pv ON pv.id=s.plan_version_id WHERE s.organization_id=$1 FOR UPDATE OF s")
