...
//! requested for on send. Both are per-user and idempotent by `mail_id`.
...
pub async fn list(State(state): State<AppState>, auth: AuthUser) -> Result<Json<Value>, ApiError> {
    let rows: Vec<ReceiptRow> = sqlx::query_as(
        "SELECT id, mail_id, sender, email, subject, at FROM read_receipts
         WHERE user_id = $1 ORDER BY at DESC LIMIT 500",
    )
    .bind(auth.user_id)
    .fetch_all(&state.db)
    .await
...
pub async fn record(
    ...
    let row = sqlx::query_as::<_, ReceiptRow>(
        "INSERT INTO read_receipts (user_id, mail_id, sender, email, subject)
         VALUES ($1, $2, $3, $4, $5)
         ON CONFLICT (user_id, mail_id) DO UPDATE SET
...
pub async fn request(
    ...
    let row = sqlx::query_as::<_, RequestRow>(
        "INSERT INTO receipt_requests (user_id, mail_id, recipient)
         VALUES ($1, $2, $3)
         ON CONFLICT (user_id, mail_id) DO UPDATE SET recipient = EXCLUDED.recipient
