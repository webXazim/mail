...
pub async fn unsubscribe(
    ...
    let email = suppression::verify_unsubscribe_token(&state.jwt_secret, &q.token)
        .ok_or_else(|| ApiError::bad_request("This unsubscribe link is invalid or has expired"))?;

    suppression::suppress(&state, &email, "unsubscribed", "unsubscribe", "").await?;

    Ok((StatusCode::OK, Html(page(&email))))
}
